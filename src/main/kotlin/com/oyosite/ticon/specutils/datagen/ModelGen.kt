...
class ModelGen(output: FabricDataOutput) : FabricModelProvider(output) {
    override fun generateBlockStateModels(blockStateModelGenerator: BlockModelGenerators) {
        SpectralUtilities.LOGGER.info("Generating Block Models...")
    ...
    /*val NOXWOOD_LAMP = Model(Optional.of(ResourceLocation("spectrum:templates/noxwood_lamp")), Optional.empty(), TextureKey.ALL)
    val NOXWOOD_LAMP_ALT = Model(Optional.of(ResourceLocation("spectrum:templates/noxwood_lamp_alt")), Optional.empty(), TextureKey.ALL)
    val NOXWOOD_LAMP_DIAGONAL = Model(Optional.of(ResourceLocation("spectrum:templates/noxwood_lamp_diagonal")), Optional.empty(), TextureKey.ALL)
    val NOXWOOD_LAMP_DIAGONAL_ALT = Model(Optional.of(ResourceLocation("spectrum:templates/noxwood_lamp_diagonal_alt")), Optional.empty(), TextureKey.ALL)
    val NOXWOOD_LAMP_HANGING = Model(Optional.of(ResourceLocation("spectrum:templates/noxwood_lamp_hanging")), Optional.empty(), TextureKey.ALL)
    val NOXWOOD_LAMP_HANGING_ALT = Model(Optional.of(ResourceLocation("spectrum:templates/noxwood_lamp_hanging_alt")), Optional.empty(), TextureKey.ALL)
    val NOXWOOD_LAMP_HANGING_DIAGONAL = Model(Optional.of(ResourceLocation("spectrum:templates/noxwood_lamp_hanging_diagonal")), Optional.empty(), TextureKey.ALL)
    val NOXWOOD_LAMP_HANGING_DIAGONAL_ALT = Model(Optional.of(ResourceLocation("spectrum:templates/noxwood_lamp_hanging_diagonal_alt")), Optional.empty(), TextureKey.ALL)
    ...
    fun noxwoodLampBlockState(lamp: Block, materials: Pair<NoxwoodDeco.NoxwoodType, NoxwoodDeco.NoxwoodType>)=
        VariantsBlockStateSupplier.create(lamp).coordinate(
            BlockStateVariantMap.create(FlexLanternBlock.TALL, FlexLanternBlock.HANGING, FlexLanternBlock.DIAGONAL).register { tall, hanging, diagonal ->
                var suffix = ""
                if(hanging)suffix+="_hanging"
                if(diagonal)suffix+="_diagonal"
                if(tall)suffix+="_tall"
    ...
    val CRYSTALLARIEUM_FARMABLE = Model(Optional.of(ResourceLocation("spectrum:block/crystallarieum_farmable")), Optional.empty(), TextureKey.CROSS)
    ...
    override fun generateItemModels(itemModelGenerator: ItemModelGenerators) {
        SpectralUtilities.LOGGER.info("Generating Item Models...")
        ItemRegistry.ENDER_FLASKS.forEach { itemModelGenerator.generateFlatItem(it, ModelTemplates.FLAT_ITEM) }
