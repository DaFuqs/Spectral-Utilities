...
class RecipeGen(output: FabricDataOutput, registryLookup: CompletableFuture<HolderLookup.Provider>) : FabricRecipeProvider(output, registryLookup) {
    override fun buildRecipes(exporter: RecipeOutput) {
        ...
        /*CrystallarieumRecipeJsonBuilder(BlockRegistry.DRAGONBONE, 240, InkColors.LIME, 4)
            ...
            .criterion("has_dragonbone", AdvancementGottenCriterion(ResourceLocation("spectrum:lategame/break_cracked_dragonbone")))
            .requiresAdvancement()
            .offerTo(exporter)*/

    }

    /*fun moonstoneGrowLampRecipe(exporter: RecipeOutput){
        PigmentPedestalRecipeJsonBuilder(BlockRegistry.MOONSTONE_GROW_LAMP).tier("complex").experience(2.0)
            ...
            .ingredient('L', SpectrumBlocks.MOONSTONE_BASALT_LIGHT)//, SpectrumBlocks.MOONSTONE_CALCITE_LAMP)
            .ingredient('C', SpectrumItems.MOONSTONE_CORE)
            .powders(1, 2, 4, 0, 4)
            .time(600)
            .criterion("can_use_moonstone_pedestal", AdvancementGottenCriterion.Conditions(Optional.empty(), ResourceLocation("spectrum:lategame/build_complex_pedestal_structure")))
            .criterion("obtained_moonstone_core", InventoryChangeTrigger.TriggerInstance.hasItems(SpectrumItems.MOONSTONE_CORE))
            .offerTo(exporter)
    }
    ...
    fun bindingToolRecipe(exporter: Consumer<RecipeJsonProvider>){
        PigmentPedestalRecipeJsonBuilder(ItemRegistry.BINDING_TOOL).tier("complex").experience(2.0)
            ...
            .time(600)
            .criterion("can_use_moonstone_pedestal", AdvancementGottenCriterion.create(ResourceLocation("spectrum:lategame/build_complex_pedestal_structure")))
            .offerTo(exporter)
    ...
    fun enderFlaskRecipe(color: DyeColor, exporter: Consumer<RecipeJsonProvider>){
        ...
        builder.tier("complex").experience(8.0).pattern("OYO","PFP","OEO")
            ...
            .powders(8, 2, 8, 16, 4)
            .time(1200)
            .criterion("gotten_radiating_ender", AdvancementGottenCriterion.create(ResourceLocation("spectrum:get_radiating_ender")))
